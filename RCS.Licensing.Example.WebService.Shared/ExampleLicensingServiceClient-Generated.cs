...
namespace RCS.Licensing.Example.WebService.Shared
{
	partial interface IExampleLicensingServiceClient
	{
		...
		Task<ResponseWrap<string?>> PasswordChangePlea(string email);
	...
	partial class ExampleLicensingServiceClient
	{
		...
		///   two-factor-authentication and then processed by an external application.
		/// </remarks>
		/// <param name="email">The account email address making a plea to change password.</param>
		[GeneratedCode("TextTemplatingService", "17.0.0.0")]
		public async Task<ResponseWrap<string?>> PasswordChangePlea(string email)
		{
			HttpResponseMessage hrm = await Client.PostAsync("user/plea/password/{email}", MakeContent(email));
			return await UnwrapAndCheckResult<string?>(hrm, throwOnError);
		}
