...
public static partial class TextConvert
{
	...
	{
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		// The serializing value musr have the expected properties and values.
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		var vt = value.GetType();
		var perror = vt.GetProperty("HasError");
		var pcode = vt.GetProperty("Code");
		var pmsg = vt.GetProperty("Message");
		var pdet = vt.GetProperty("Detail");
		var pdata = vt.GetProperty("Data");
		if (perror == null || pcode == null || pmsg == null || pdet == null || pdata == null) throw new NotSupportedException($"Type {vt} cannot be serialized to text lines because it is not a supported response type.");
		bool? hasError = (bool?)perror.GetValue(value) ?? throw new NotSupportedException($"Property {vt}.HasError does not contain a value");
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		// Serialize expected properties.
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		lines.Add($"HasError={hasError}");
		lines.Add($"Code={pcode.GetValue(value)}");
		lines.Add($"Message={pmsg.GetValue(value)}");
		lines.Add($"Detail={pdet.GetValue(value)}");
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		// Serialize Data via dynamic dispatch.
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		object? data = pdata.GetValue(value);
		if (data != null)
		{
			lines.Add($"# Text serialize type {data.GetType().FullName}");
			SerializeData((dynamic)data, lines);   // <=========== DYNAMIC DISPATCH =============
		}
	}

	/// <summary>
	/// Default text serializer for an arbitrary class object. Creating an override for specific value types allows custom serialization.
	/// </summary>
	static void SerializeData(object value, Lines lines)
	{
		...
		foreach (string line in lines)
		{
			...
			if (rawvalue.Length > 0)
			{
				...
				else if (pinf.PropertyType == typeof(DateTime) || pinf.PropertyType == typeof(DateTime?))
				{
					...
					value = DateTime.ParseExact(rawvalue, Program.TextSerializeDateFormat, DateTimeFormatInfo.InvariantInfo);
				}
				else
				{
